...
const AppointmentLogReport = () => {
  const navigate = useNavigate();
  const { persons, getAllPersons } = usePersonStore();
  const { vehicles, getAllVehicles } = useVehicleStore();
  const { appointments, generateAppointmentsReport, pagination, isLoading } =
    useAppointmentStore();
  ...
  const generateReport = async () => {
    const response = await generateAppointmentsReport({
      fromDate: fromDate || undefined,
      toDate: toDate || undefined,
      type: type || undefined,
      personId: personId || undefined,
  ...
  const fetchAppoinmentsLogs = async () => {
    await generateAppointmentsReport(buildParams());
  };
  ...
  return (
    <>
      ...
      <div className='flex items-center justify-between mb-6'>
        <div>
          <h1 className='text-2xl font-semibold'>Appointment Logs Report</h1>
          <p className='text-sm text-muted-foreground'>
            Manage organizational Appointment Logs
          </p>
        </div>
        ...
        <Card>
          <CardContent>
            ...
            <div className='flex gap-2 mt-4'>
              <Button onClick={fetchAppoinmentsLogs} disabled={isLoading}>
...
export default AppointmentLogReport;
